...
func (cli *Client) parseResponse(resp *http.Response) ([]byte, error) {
	...
	if errors.Is(err, errUnauthorized) {
		return nil, cli.errorOut(multierr.Append(err, fmt.Errorf("your credentials may be missing, invalid or you may need to login first using the CLI via 'plugin admin login'")))
	}
